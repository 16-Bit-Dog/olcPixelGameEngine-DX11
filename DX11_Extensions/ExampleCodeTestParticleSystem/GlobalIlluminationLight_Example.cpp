...
public:
	...
	bool OnUserCreate() override
	{
		...
		HandleToBasicPointLight2 = DX11CreateBasicPointLight(0.5f, { 50,100 }, 0.5f, olc::WHITE, { 400,325 }, 0.0f);



		HandleToBasicPointLight5 = DX11CreateBasicPointLight(0.5f, { 50,100 }, 0.5f, olc::WHITE, { 400,100 }, 0.0f);

		HandleToBasicPointLight6 = DX11CreateBasicPointLight(0.5f, { 50,100 }, 0.5f, olc::WHITE, { 425,150 }, 0.0f);
	...
	//	}
		counter += 0.1;
		//GlobalIlluminationImediateChange(olc::Pixel(255, 255, 255, 255));
		...
		if (counter >= 10) {
			if (falser == false) {
 				GlobalIlluminationResetChange(olc::Pixel(i, i, i,255)); //notice how this has to be used later in run time rather than immediatly - this is because of how DX11 runtime works... need to give it a bit - I did not add sync to stop this, so you need to wait a few seconds before calling
				falser = true;
				i += 20;
				if (i > 255) {
					i = 0;
