...
public:
	...
	bool OnUserCreate() override
	{
		...
		ComputeFloatBasicHandle = DX11CreateVecBasicComputeFloat(vec1.size(), { vec1, vec2 });  //has vec0, vec1
		
		AdjustVecBasicFloat(ComputeFloatBasicHandle, vec1.size(), { vec1, vec2, vec1 });  // has vec0, vec1, vec2 after adjustment

		NewMathForVecBasicFloat(ComputeFloatBasicHandle, "vec0+vec1+vec2"); 
